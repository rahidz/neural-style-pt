...
class GramMatrix(nn.Module):
    ...
    def forward(self, input):
        N, C, H, W = input.size()
        x_flat = input.view(N, C, H * W)
        return torch.matmul(x_flat, x_flat.transpose(1, 2)) 
...
class CovarianceMatrix(nn.Module):
    ...
    def forward(self, input):
        N, C, H, W = input.size()
        x_flat = input.view(N, C, H * W)
        x_flat = x_flat - x_flat.mean(2).unsqueeze(2)
        return torch.matmul(x_flat, x_flat.transpose(1, 2))
...
class StyleLoss(nn.Module):
    ...
    def __init__(self, strength, style_stat, style_masks, content_masks):
        ...
        self.style_masks = [torch.cat([m.repeat(1,1,1) for m in style_mask]) for style_mask in copy.deepcopy(style_masks)] #copy.deepcopy(style_masks)
        self.content_masks = torch.cat([m.repeat(1,1,1) for m in copy.deepcopy(content_masks)]) #copy.deepcopy(content_masks)
    ...
    def forward(self, input):
        ...
        if self.mode != 'none':
            loss = 0
            l_means = torch.mean(masks, axis=([1,2])) + 1e-24
            masks = masks.unsqueeze(1).repeat(1,input.shape[-3],1,1)
            masked_features = masks.mul(input)
            masked_grams = self.gram(masked_features)
            masked_grams = masked_grams.div(input.nelement() * l_means[:, None, None])
            if self.mode == 'capture':
                if self.target_grams is None:
                    self.target_grams = masked_grams.detach().mul(self.blend_weight)
                else:
                    self.target_grams += masked_grams.detach().mul(self.blend_weight)
            elif self.mode == 'loss':
                #loss = torch.sum(self.crit(self.target_grams, masked_grams).mul(l_means[:, None])) * self.strength
                loss = sum([self.crit(self.target_grams[i], masked_grams[i]) * l_means[i] * self.strength 
...
class HistLoss(nn.Module):
    ...
    def __init__(self, strength, style_masks, content_masks):
        ...
        self.mode = 'none'
        self.blend_weight = 1.0
        self.style_masks = [torch.cat([m.repeat(1,1,1) for m in style_mask]) for style_mask in style_masks]
        self.content_masks = torch.cat([m.repeat(1,1,1) for m in content_masks])
        self.capture_count = 0
        self.num_styles = len(self.style_masks)
    ...
    def forward(self, input):
        if self.mode == 'capture':
            masks = self.style_masks[self.capture_count]
            masks2 = self.style_masks[self.capture_count]
            self.capture_count += 1
        elif self.mode == 'loss':
            masks = self.content_masks
            masks2 = self.content_masks
            self.style_masks = None
        if self.mode != 'none':
            loss = 0
            masks = masks.unsqueeze(1).repeat(1,input.shape[-3],1,1)
            ...
            for j in range(self.num_styles):
                target_hist = cpp.computeHistogram(masked_features[j], 256)
                if self.mode == 'capture':		
                    target_min, target_max = self.minmax(masked_features[j].unsqueeze(0))
                    ...
                    target = self.calcHist(masked_features[j], self.target_hists[j], self.target_mins[j], self.target_maxs[j])
                    loss += 0.01 * self.strength * self.crit(masked_features[j].unsqueeze(0), target)
            self.loss = loss
        return input
